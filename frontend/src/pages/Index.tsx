...
const Index = () => {
  ...
  // Fetch welcome messages from CMS settings
  useEffect(() => {
    fetch(`${API_BASE}/settings`)
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data?.welcomeMessages?.length > 0) {
          setVibes(data.welcomeMessages);
        }
      })
      .catch(() => {}); // fallback to default silently
    setVibeIdx(Math.floor(Math.random() * DEFAULT_VIBES.length));
  }, []);
    ...
    <div className="min-h-screen bg-[#F7F2EF] flex flex-col items-center pt-8 pb-14 px-4">
      ...
      <div className="w-full max-w-[360px] bg-white rounded-3xl shadow-lg shadow-stone-200/60 border border-stone-100 overflow-hidden">
        ...
        <div className="pt-8 pb-5 px-7 text-center">
          ...
          {/* Script logo */}
          <h1
            className="font-['Great_Vibes'] text-[64px] text-primary leading-[1.1] select-none"
          ...
          <div className="grid grid-cols-3 divide-x divide-stone-100 border border-stone-100 rounded-2xl overflow-hidden text-[10px]">
            <a
              href="https://maps.app.goo.gl/DruZXXTrtSVBj6LW9"
              target="_blank"
            ...
            >
              ...
              <span className="text-stone-500 text-center leading-[1.4] group-hover:text-primary transition-colors duration-150">
                64 Linh Trung<br />TP. Hồ Chí Minh
              </span>
            </a>
            <a
              href="tel:0358367919"
            ...
            >
              ...
              <span className="text-stone-500 leading-[1.4] group-hover:text-primary transition-colors duration-150">
                035 836 7919
              </span>
            </a>
            <div className="flex flex-col items-center gap-1.5 py-3 px-1">
              <Clock className="w-3.5 h-3.5 text-primary shrink-0" />
              <span className="text-stone-500 text-center leading-[1.4]">
                08:00<br />20:30
              </span>
            </div>
          </div>
        ...
        <div className="px-5 pb-6 space-y-2.5">
          ...
          >
            ...
            <div className="flex-1 text-left">
              <p className="font-bold text-[13px] text-white leading-tight">Đặt Lịch Online</p>
              <p className="text-[10px] text-white/65 mt-0.5">Chọn giờ — em confirm ngay!</p>
            </div>
          ...
          {[
            {
              href: "https://maps.app.goo.gl/DruZXXTrtSVBj6LW9",
              icon: <MapPin className="w-[17px] h-[17px] text-primary" />,
              label: "Chỉ Đường Đến Tiệm",
              sub: "64 Linh Trung, Linh Xuân, TP.HCM",
            },
            {
              href: "https://www.facebook.com/thai.ngoc.quynh.nhu?locale=vi_VN",
              icon: <Facebook className="w-[17px] h-[17px] text-primary" />,
              label: "Facebook Tiệm",
              sub: "Follow để xem inspo nails mới nhất!",
            },
          ].map(item => (
            <a
              key={item.href}
